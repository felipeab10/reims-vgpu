...
generate_opencore(){ base=$1; sw="$base/serial-work"; mkdir -p "$sw/.fish" "$sw/staged" "$base/serial"; cp "$ROOT/third_party/osx-serial-generator/opencore-image-ng-linux.sh" "$sw/opencore-image-ng.sh"; cp -a "$OSX_KVM/OpenCore/EFI" "$sw/staged/"; sed -i "s#cp -a \"\${BASE}/EFI\" \"\${WORK}\"#cp -a \"\${BASE}/../staged/EFI\" \"\${WORK}\"#" "$sw/opencore-image-ng.sh"; cp -a "$OSX_KVM/OpenCore/startup.nsh" "$sw/.fish/" 2>/dev/null || :; (cd "$sw" && bash "$ROOT/third_party/osx-serial-generator/generate-unique-machine-values.sh" --count 1 --model "$SMBIOS_MODEL" --width 1920 --height 1080 --output-dir "$base/serial" --master-plist "$ROOT/third_party/osx-serial-generator/config-custom.plist" --create-envs --create-plists); generated=$(find "$base/serial/plists" -type f -name '*.plist' -print -quit); [[ -n "$generated" ]]||die plist; cp "$generated" "$base/serial/config-auto.plist"; python3 - "$base/serial/config-auto.plist" <<'PY'
