...
generate_opencore(){ base=$1; sw="$base/serial-work"; mkdir -p "$sw/.fish" "$base/serial"; cp "$ROOT/third_party/osx-serial-generator/opencore-image-ng-linux.sh" "$sw/opencore-image-ng.sh"; cp -a "$OSX_KVM/OpenCore/EFI" "$sw/.fish/"; cp -a "$OSX_KVM/OpenCore/startup.nsh" "$sw/.fish/" 2>/dev/null || :; (cd "$sw" && bash "$ROOT/third_party/osx-serial-generator/generate-unique-machine-values.sh" --count 1 --model "$SMBIOS_MODEL" --width 1920 --height 1080 --output-dir "$base/serial" --master-plist "$ROOT/third_party/osx-serial-generator/config-custom.plist" --create-envs --create-plists); generated=$(find "$base/serial/plists" -type f -name '*.plist' -print -quit); [[ -n "$generated" ]]||die plist; cp "$generated" "$base/serial/config-auto.plist"; python3 - "$base/serial/config-auto.plist" <<'PY'
...
PY
(cd "$sw" && bash "$sw/opencore-image-ng.sh" --img "$base/persistent/OpenCore.qcow2" --cfg "$base/serial/config-auto.plist"); printf 'model=%s\nplist=%s\n' "$SMBIOS_MODEL" "$generated" > "$base/serial-generator.txt"; }
